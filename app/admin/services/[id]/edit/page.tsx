...
import { updateServiceAction, getServiceAction, getCategoriesAction } from "@/app/admin/services/actions";
...
export default function EditServicePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [cats, service] = await Promise.all([
          getCategoriesAction(),
          getServiceAction(Number(params.id)),
        ]);
        setCatList(cats);
        ...
        if (service) {
          ...
          setSpecs(service.specs?.length ? service.specs : [""]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateServiceAction(Number(params.id), {
        nameEn: form.nameEn,
        nameAr: form.nameAr,
        slug: form.slug,
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="h-[600px] bg-surface animate-pulse rounded-2xl" />
      </div>
    );
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
        <div className="bg-surface rounded-2xl border border-border p-6 space-y-4">
          <h2 className="text-sm font-semibold text-text uppercase tracking-wider">English</h2>
          <div>
        ...
        <div className="bg-surface rounded-2xl border border-border p-6 space-y-4">
          <h2 className="text-sm font-semibold text-text uppercase tracking-wider">العربية</h2>
          <div>
            <label className="block text-sm font-medium text-text mb-1.5">
          ...
          <div>
            <label className="block text-sm font-medium text-text mb-1.5">الوصف</label>
            <textarea
        ...
        <div className="bg-surface rounded-2xl border border-border p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">Original Price</label>
              <input
                type="number"
                step="0.001"
                min="0"
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                ...
                >
                  {tag}
                  <button type="button" onClick={() => removeTag(tag)} className="text-text-muted hover:text-accent">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
